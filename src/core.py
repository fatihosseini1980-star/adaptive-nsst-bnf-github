...
Model for the adaptive NSST-BNF:
    y = m_theta(x) + omega^{-1/2} { delta_psi(x) U + epsilon }
    epsilon ~ N(0, C_eta)
    U ~ HalfNormal(1)
...
class AdaptiveNSSTBNF(nn.Module):
    ...
    def fields(self, coords, sample=True):
        ...
        alpha_amp, _ = self.q_alpha_amp.rsample() if sample else (
            torch.exp(self.q_alpha_amp.log_mean), None
        )
        phi_amp, _ = self.q_phi_amp.rsample() if sample else (
            torch.exp(self.q_phi_amp.log_mean), None
        )

        g = self.alpha_net.centered_output(coords, sample=sample)
        h = self.phi_net.centered_output(coords, sample=sample)

        alpha = alpha0 + alpha_amp * g
        delta = alpha / torch.sqrt(1.0 + alpha**2)

        # Global positive range plus centered local log-range deviations.
    ...
    @torch.no_grad()
    def posterior_surface_summary(self, coords, n_draws=200):
        alphas, phis = [], []
        for _ in range(n_draws):
            _, a, _, p = self.fields(coords, sample=True)
            alphas.append(a.cpu().numpy())
            phis.append(p.cpu().numpy())
        return np.mean(alphas, axis=0), np.mean(phis, axis=0)

    @torch.no_grad()
    def predictive_samples(self, y_train, x_train, x_test, n_draws=1000):
        out = []
        # Joint centering must be computed on the combined design so that train
        # and test local deviations use a common empirical reference level.
        xall = torch.cat([x_train, x_test], dim=0)
        ntr = x_train.shape[0]
        for _ in range(n_draws):
            mall, aall, dall, pall = self.fields(xall, sample=True)
...
class StationarySkewTBNF(nn.Module):
    ...
    def predictive_samples(self, y_train, x_train, x_test, n_draws=1000):
        out=[]
        for _ in range(n_draws):
            mtr=self.mean_net(x_train, sample=True).squeeze(-1)
            mte=self.mean_net(x_test, sample=True).squeeze(-1)
